...
module.exports = function(proto) {
  ...
  proto.exec =
  proto.execute =
  proto.run = function(args,options) {
    var self = this;
    self.options=options;

    //Add input for progression if there is not
    // FFprobe will run on it
    if(!self._inputs || self._inputs.length == 0){
      var idx = args.indexOf('-i');
      if(idx < 0){
        throw new Error('No input found in command line: -i missing');
      }
      self._inputs = [];
      var input = {};
      input.isFile = true;
      input.isStream = false;
      input.source = args[idx+1];
    ...
    //Run ffprobe to get progression
    if (this.listeners('progress').length > 0) {
      // Read metadata in parallel
      runFfprobe(this);
    } else {
    ...
    //  }
      ...
      self._spawnFfmpeg(
        ...
        options,
        // {
        //   captureStdout: false,//!outputStream,
